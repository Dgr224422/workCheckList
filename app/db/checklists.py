...
async def mark_run_step(run_step_id: int, photo_path: str | None, done_at: str) -> None:
    conn = await connect("checklists.db")
    await conn.execute(
...
async def add_reminder(worker_id: int, title: str, weekday: int | None, exact_date: str | None, created_at: str) -> None:
    conn = await connect("checklists.db")
    await conn.execute(
